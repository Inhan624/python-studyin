...
# 1. enumerate()함수 - 리스트
# 리스트에 저장된 요소와 해당 요소의 인댁스가 튜플(tuple)형태로 함께 추출
...
# 2. range() 함수
# 숫자 리스트를 생성하는데 사용
# 주로 for 반복만에서 사용, 일정한 각격의 숫자 시퀀스를 생성가능

 # range(stop):
 for i range(5):
     print(i)

 # rnage(start, stop): start ~ stop-1
 for i range(1, 5):
     print(i)
...
for i in result:
    print(i)
# 결괏값: 각각의 숫회 가능한 값의 길이가 다를 경우
# 동일한 인덱스 변호의 값만을 추출 (가장 짧은 길이의 값을 기준으로 튜플을 반환)
#
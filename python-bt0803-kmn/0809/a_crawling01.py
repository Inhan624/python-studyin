...
#! 2. 웹 크롤링 목적
# 2-1. 검색 엔진 최적화(SEO)
# : 웹 크롤러는 검색 엔진에서 웹페이지의 내용을 인덱싱(추출)하기 위해 사용
# 2-2. 데이터 수집
...
# 4-2. HTTP 요청과 응답
# 요청(Request): 웹 브라우저(클로저)가 서버에게 정보나 서비스를 요청하는 메시지
# 응답(Response): 서버가 요청을 처리한 후 반환하는 메시지

# 4-3. HTTP메서드
# GET: 웹페이지의 내용을 조회할 때 사용, 웹크롤링에서 가장 기본적인 요청방식
# POST: 서버에 데이터를 보낼 때 사용

#  4-4. HTTP 상태 코드(크롤링 중 발생할 수 있는 상태)
# 200 OK: 성공적으로 응답 받았음.(크롤링 대상 데이터를 가져오기 가능)
# 404 Forbidden: 접근 권한이 없음.(로봇 차단 정책 등으로 크롤링 제한)
# 404 Not Found
# : 요청한 URL에 해당하는 페이지가 없음.(링크가 끊긴 페이지 참조시 발생)
...
#! robots.txt
# : 웹사이트의 어는 부분을 수집하거나 수집하지 않아야 하는지에 대한 지침을 제공
